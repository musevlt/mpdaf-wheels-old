###################################
Building and uploading mpdaf wheels
###################################
...
https://travis-ci.org/musevlt/mpdaf-wheels

.. Appveyor interface at
.. https://ci.appveyor.com/project/MacPython/mpdaf-wheels

The driving github repository is
https://github.com/musevlt/mpdaf-wheels
...
* builds a mpdaf wheel, linking against these fresh builds;
...
* making a commit to the `mpdaf-wheels` repository (e.g. with `git
...
Which mpdaf commit does the repository build?
===============================================

The `mpdaf-wheels` repository will build the commit specified in the
...
be something like::
    ...
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t macosx mpdaf $VERSION
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t manylinux1 mpdaf $VERSION
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t win mpdaf $VERSION
...
`mpdaf` is the root name of the wheel(s) to download / upload and `1.3.2` is
...
`mpdaf-1.3.2-` from the URL in ``$CDN_URL`` above to ``~/wheelhouse``, then
upload them to pypi.
